...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    <div class="entry-wrap">
        <?php $post_audio_url = rwmb_meta( 'rt_post_audio_url' ); ?>
        <?php $post_audio_embeded_code = rwmb_meta( 'rt_post_audio_embeded_code' ); ?>


        <?php if ( !empty($post_audio_url) ) { ?>
        <div class="entry-thumb">
            <?php echo do_shortcode('[audio src="' . $post_audio_url . '"]'); ?>
        </div>
        <?php } else { ?>
        <?php  if ( !empty($post_audio_embeded_code) ) { ?>
        <div class="entry-thumb">
            <?php echo $post_audio_embeded_code; ?>
        </div>
        <?php } ?>
        <?php } ?>
        ...
        <footer class="entry-meta">
            <span class="post-format">
                <i class="fa fa-music"></i> <a href="<?php echo esc_url( get_post_format_link( 'audio' ) ); ?>"><?php echo get_post_format_string( 'audio' ); ?></a>
            </span>
