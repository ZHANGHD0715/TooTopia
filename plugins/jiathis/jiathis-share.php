...
add_filter('wp_footer','jiathis_footer');
function jiathis_footer(){
	global $jiathis_feed,$jiathis_code;
	$partten = '/jiathis_r.js/';
	$result = preg_match($partten,$jiathis_code,$res);
	if($jiathis_feed == 'yes' && $result){
		echo '<style>#ckepop table tr td {border:none;}#ckepop table{border:none;}</style>';
		echo $jiathis_code;
	}
}
...
function jiathisActionLinks($links, $file) {
		array_unshift($links, '<a href="options-general.php?page=jiathis-share.php">'.__('Settings').'</a>');
	    return $links;
}
...
function jiathis_options() {
	...
	echo '提示：全站显示只针对于侧栏式起作用，如果选择了全站显示，请你将JiaThis代码替换为侧栏式代码。<br><br>';
	echo '文章头部 ：&nbsp;&nbsp;&nbsp;';
