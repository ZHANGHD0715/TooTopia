...
function wp_rp_generate_related_posts_list_items($related_posts, $selected_related_posts, $post_categories) {
	...
	for ($i = 0; $i < $limit; $i++) {
		...
		$data_attrs = 'data-position="' . $i . '" data-poid="' . $post_id . '" data-post-type="' . $post_type . '" '; // Deprecated


		//$output .= '<li ' . $data_attrs . '>';
		$output .= '<div class="col-xs-12 col-md-6 col-lg-4 article-wrap isotope-item"><article class="post type-post status-publish format-standard sticky hentry category-uncategorized">';

		$post_url = property_exists($related_post, 'post_url') ? $related_post->post_url : get_permalink($related_post->ID);

		$output .= '<div class="entry-wrap panel">';
		//image
		$output .=  wp_rp_get_post_thumbnail_img($related_post);
		// if ($img) {
		...
		if ($platform_options["display_category"] && !empty($post_categories)){
			...
			foreach($post_categories[$id] as $cat_id => $cat_name) {
				...
				$output .= '<span class="post-cats"><i class="fa fa-th-list"></i> ' . '<a href="' . $cat_url . '" rel="category tag">' . $cat_name . '</a>' . '</span>';
				//$output .= '<a href="' . $cat_url . '" target="_parent">' . $cat_name . '</a>';
				$num_categories++;
			}
		}

		$output .= '<span class="post-date"><i class="fa fa-clock-o"></i> 
		<a href="' . $post_url . '"  rel="bookmark">
		<time class="entry-date" datetime="'. $related_post->post_date .'">'. mysql2date($dateformat, $related_post->post_date) .'
		</time></a></span></footer></div></article></div>';
