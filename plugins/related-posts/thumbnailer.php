...
function wp_rp_get_post_thumbnail_img($related_post, $size = null, $force = false) {
	...
	$img_dom = '<div class="post-thumb entry-thumb" data-test="' . $related_post->ID  . '">';
	$options = wp_rp_get_options();
	$platform_options = wp_rp_get_platform_options();
	...
	if ($options['thumbnail_use_custom']) {
		...
		if ($thumbnail_src) {
			...
			return $img_dom .= '<img src="'. esc_attr($thumbnail_src) . '" alt="' . esc_attr($post_title) . '" /> </div>';
		}
	}
	...
	if ($featured_image) {
		...
		if ($featured_image_thumb) {
			...
			return $img_dom .= '<img src="'. esc_attr($featured_image_thumb["url"]) . '" alt="' . esc_attr($post_title) . '" /> </div>';
		} 
		else if($post_thumbnail_url) {
			//return get_the_post_thumbnail($post_id, $size);
			return $img_dom .= '<img src="'. esc_attr($post_thumbnail_url) . '" alt="' . esc_attr($post_title) . '" /> </div>';
		}
    }

	$attached_img_url = wp_rp_get_attached_img_url($related_post, $size);
	if ($attached_img_url) {
		return $img_dom .= '<img src="'. esc_attr($attached_img_url) . '" alt="' . esc_attr($post_title) . '" /> </div>';
	} else {
		$attached_img_url = wp_rp_get_default_thumbnail_url($related_post->ID, $size);
		return $img_dom .= '<img src="'. esc_attr($attached_img_url) . '" alt="' . esc_attr($post_title) . '" /> </div>';
	}
